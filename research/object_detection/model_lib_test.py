        ...
        with tf.Graph().as_default():
            ...
            if mode == 'eval' or mode == 'eval_on_train':
                if class_agnostic:
                    ...
                    self.assertEqual(
                        batch_size, detection_classes.shape.as_list()[0])
                ...
                self.assertEqual(
                    batch_size, detection_boxes.shape.as_list()[0])
                self.assertEqual(tf.float32, detection_boxes.dtype)
                self.assertEqual(
                    batch_size, detection_scores.shape.as_list()[0])
...
class UnbatchTensorsTest(tf.test.TestCase):
    ...
    def test_unbatch_without_unpadding(self):
        ...
        groundtruth_boxes_placeholder = tf.placeholder(
            tf.float32, [2, None, None])
