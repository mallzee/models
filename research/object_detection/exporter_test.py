...
class FakeModel(model.DetectionModel):
    ...
    def __init__(self, add_detection_keypoints=False,
    ...
    def test_export_and_run_inference_with_image_tensor(self):
        ...
        with self.test_session(graph=inference_graph) as sess:
            ...
            self.assertAllClose(classes_np, [[1, 2],
                                             [2, 1]])
            self.assertAllClose(
                keypoints_np, np.arange(48).reshape([2, 2, 6, 2]))
        ...
        with self.test_session(graph=inference_graph) as sess:
            ...
            for image_str in [jpg_image_str, png_image_str]:
                ...
                self.assertAllClose(
                    keypoints_np, np.arange(48).reshape([2, 2, 6, 2]))
                self.assertAllClose(
                    masks_np, np.arange(64).reshape([2, 2, 4, 4]))
                self.assertAllClose(num_detections_np, [2, 1])
        ...
        with mock.patch.object(
            ...
            with tf.gfile.GFile(expected_pipeline_path, 'r') as f:
                ...
                self.assertProtoEquals(
                    pipeline_config, written_pipeline_config)
